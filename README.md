...
为了使程序脱离 Python 环境独立在桌面端双击运行，项目已集成基于 PyInstaller 的跨平台一键打包方案，并自动关联精美的艺术 Logo 图标（落日橙红与金色波纹微拟物风格）：

### 1. 自动转换生成图标
项目中集成了 `convert_icon.py` 转换工具。运行打包脚本时，系统会自动利用 `Pillow` 将原始图标源文件转换并输出：
- **`gui/icon.png`** (256x256，用于 PyQt/PySide6 运行态窗口左上角图标)
- **`icon.ico`** (Windows 专属多尺寸复合图标)
- **`icon.icns`** (macOS 专属图标文件)

### 2. macOS 平台打包 (.app 专属格式)
在 macOS 本地终端运行以下命令：
```bash
sh build_mac.sh
```
该脚本会自动安装 `Pillow` 和 `pyinstaller`，生成对应格式图标，并执行 PyInstaller 封装。打包成功后，可在 **`dist/`** 目录下获取 **`JAV SCRAPER.app`**（双击即可直接运行）和独立二进制文件夹。

### 3. Windows 平台打包 (.exe 专属格式)
由于打包不支持跨系统交叉编译，Windows 可执行程序的打包需要在一台 Windows 电脑上进行。
在 Windows 终端中运行或直接双击项目根目录下的：
```bat
build_win.bat
```
脚本会自动配置 Windows 平台依赖并生成包含专属图标的 `.exe` 独立应用程序，输出于 `dist/` 目录。
